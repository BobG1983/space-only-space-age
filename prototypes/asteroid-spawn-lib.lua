-- The spawn-definition helper for six classes plus promethium: a copy of
-- space-age/prototypes/planet/asteroid-spawn-definitions.lua:239-442 with a 7-slot ratio vector
-- and the type id taken from the position in the type list. Satisfies DESIGN.md section 1.
-- Speed and angles come from the routes file so both files read the same numbers.

local routes = require("__space-only-space-age__.prototypes.asteroid-spawn-routes")

local asteroid_functions = {}

asteroid_functions.standard_speed = routes.standard_speed
asteroid_functions.chunk_angle = routes.chunk_angle
asteroid_functions.small_angle = routes.small_angle
asteroid_functions.medium_angle = routes.medium_angle
asteroid_functions.big_angle = routes.big_angle
asteroid_functions.huge_angle = routes.huge_angle
...
asteroid_functions.interpolate_point_ratio = function(pointA, pointB, position)
  local interpolated_data = {position = position, ratios = {1,1,1,1,1,1,1}}
...
asteroid_functions.spawn_definitions = function(data, planet)
  ...
  local asteroid_types = {"s", "m", "c", "v", "e", "d"}
  if data.has_promethium_asteroids == true then
    asteroid_types = {"s", "m", "c", "v", "e", "d", "promethium"}
  end
  for k, asteroid_size in ipairs(asteroid_sizes) do
    -- the type id is its slot in the ratio vector
    for asteroid_id, asteroid_type in ipairs(asteroid_types) do
      local asteroid_name = ""
